...
// use crate::{models::group::{GroupRequest, GroupSummary}, server::AppState};
...
//     if let Some(group) = groups.iter().find(|g| g.owner == payload.owner && g.id == payload.group_id).cloned() {
